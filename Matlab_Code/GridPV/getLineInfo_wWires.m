...
function Lines = getLineInfo_wWires(DSSCircObj, DSSText, varargin)
%% Parse inputs
...
for ii=1:length(Lines)
    ...
    %DJM pull out wire data and Switched status
    DSSText.command = ['Select Line.' Lines(ii).name];
    DSSText.command = ['? wires'];
    Lines(ii).wires = DSSText.Result;

    DSSText.command = '? switch';
    Lines(ii).SwitchStatus = DSSText.Result;
    if isempty(Lines(ii).wires)
        Lines(ii).wiretype = Lines(ii).lineCode;
    elseif strcmp(Lines(ii).SwitchStatus,'False')
        Lines(ii).wiretype = Lines(ii).wires(1:strfind(Lines(ii).wires,' ')-1);
    end
