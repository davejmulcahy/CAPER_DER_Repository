function [tempLines] = addAllDownstream_wup(Lines, Transformers)
...
%         upBuses = [upBuses; tempLines.bus1];
    ...
    [tempLines(traceIdx).parentBuses] = arrayfun(@(x)...
                                                    setdiff(union(tempLines(idx).parentBuses,preBuses,'legacy'), tempLines(x).childBuses),...
                                                    traceIdx);
    tempLines(idx).childElements = union(tempLines(idx).childElements, preNames) ;
